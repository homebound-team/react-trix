...
var TrixEditor = (function (_super) {
    ...
    TrixEditor.prototype.handleChange = function (e) {
        ...
        var filterMentions = function (text) {
            var _a = __read(props.mergeTags, 1), _b = _a[0], tags = _b.tags, trigger = _b.trigger;
            var mentionHint = text
                .slice(text.lastIndexOf("@"))
                .replace(/\xA0/, " ");
            if (mentionHint) {
                var filteredTags = tags.filter(function (_a) {
                    var tag = _a.tag;
                    return tag.startsWith(mentionHint);
                });
